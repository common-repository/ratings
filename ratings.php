...
if ( ! class_exists( 'Ratings' ) ) {
    class Ratings extends WP_Widget
    {
        ...
        'default_message'           => '<h3>To get it work, please add "Rarings area" widget to your theme.</h3><br />Open "<a href="$url$">$widget$</a>" and find "<strong>Rarings area</strong>" widget, then drag & drop it to the right side to activate the ratings widget.<br /> After "Rarings area" widget was activated you will be able to make appropreate rating widget configuration and the results of ratings will be visible on this plagin page.',
        )
        );
        ...
        public function result_page(){
            ...
            if (isset($_GET['widget_id'])) {
                $widgetGet  = sanitize_text_field( $_GET['widget_id'] );
            }
            $rateGet    = @(int)$_GET['rate'];
            
            $formAction = '';
            if (isset($_POST['form_action'])) {
                $formAction = sanitize_text_field( $_POST['form_action'] );
            }
            
            if (isset($_POST['item_list'])) {
                $item_list = sanitize_text_field( $_POST['form_action'] );
            }
            $item_list  = @$_POST['item_list'];
            
            if (isset($_POST['widget_id'])) {
                $widgetPost = sanitize_text_field( $_POST['widget_id'] );
            }
            ...
            if($formAction == 'delete_box' && !empty( $widgetPost ) ){
        ...
        public function frontend_wp_head(){
            ...
            wp_localize_script( 'ratings-widget', 'ratingsSetting', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ))); 
